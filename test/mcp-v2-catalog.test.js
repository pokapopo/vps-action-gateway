...
test("catalog exposes exactly nine stable core tools with loose selectors", () => {
  const tools = getCoreCatalog();
  assert.deepEqual(tools.map((tool) => tool.name), ["health", "read", "observe", "edit", "move_out", "execute", "manage", "operation", "discover"]);
